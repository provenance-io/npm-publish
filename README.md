...
const balance = await client.cosmos.bank.v1beta1
    .allBalances({ address: 'provenance1addresshere' });

// you can also query the provenance modules
const balances = await client.provenance.exchange.v1beta1
    .exchangeBalances()
```
...
const {
    ...
    deposit
} = provenance.exchange.v1beta1.MessageComposer.withTypeUrl;
```
...
const mnemonic =
  ...
  const chain = chains.find(({ chain_name }) => chain_name === 'provenance');
...
Contract schemas live in `./contracts`, and protos in `./proto`. Look inside of `scripts/codegen.js` and configure the settings for bundling your SDK and contracts into `provenance`:
